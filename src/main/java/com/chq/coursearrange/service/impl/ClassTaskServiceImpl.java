...
try {
     ...
     * 	上课时间：2
     * 	教室编号：6
     * 编码规则为：是否固定+年级编号+班级编号+教师编号+课程编号+课程属性+上课时间， 共32位
     * 其中如果不固定开课时间默认填充为"00"
     * 经过处理后得到开课任务中
     * @param classTaskList
     * @return List<String>
    ...
    private Map<String, List<String>> geneticEvolution(Map<String, List<String>> individualMap) {
        ...
        for (int i = 0; i < generation; ++i) {
            ...
            // 2、合拢所有班级的个体
            List<String> list = collectGene(individualMap);
            // 3、变异
            resultGeneList = geneMutation(list);
            // 4、冲突检测，消除冲突
            List<String> noConflictList = conflictResolution(resultGeneList);
            // 5、将消除冲突后的个体再分班进入下一次进化
            individualMap = transformIndividual(noConflictList);
        }
        return individualMap;
    }
