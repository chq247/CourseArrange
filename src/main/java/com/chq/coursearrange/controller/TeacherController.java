...
import com.chq.coursearrange.entity.request.TeacherAddRequest;
...
public class TeacherController {
    ...
    public ServerResponse selectTeacherAll(){
        List<Teacher> teacherList = teacherService.selectTeacherAll();
        if (!teacherList.isEmpty() && teacherList.size()!=0){
            return ServerResponse.ofSuccess(teacherList);
        }
     ...
     * @return
     */
    @PostMapping("/selectTeacherNo")
    public ServerResponse selectTeacherNo(@RequestBody String TeacherName) throws UnsupportedEncodingException {
        String decode = URLDecoder.decode(TeacherName,"UTF-8");
        String substring = decode.substring(0, decode.length() - 1);
        Teacher teacher = teacherService.selectByRealName(substring);
        if (teacher!=null){
